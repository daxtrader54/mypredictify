...
- `data/memory/elo-ratings.json` — current Elo ratings (updated by evaluate-results)
- `data/memory/signal-weights.json` — learned signal weights + model blend weights
- `data/memory/poisson-calibration.json` — per-league goal prediction bias correction
- `data/memory/heuristics.md` — learned patterns from past evaluations
...
- Read `data/memory/signal-weights.json` (includes learned `modelWeights` for Elo/Poisson/Odds blend)
- Read `data/memory/poisson-calibration.json` (per-league goal bias correction)
- Read `data/memory/heuristics.md`
- Read `data/config/pipeline.json`
- Read `data/memory/prompt-fragments/synthesis.md` (if exists)

**Learned State**: The `generate-predictions.mjs` script automatically reads:
- `modelWeights` from `signal-weights.json` (default: Elo 30%, Poisson 30%, Odds 40%)
- `poisson-calibration.json` for per-league goal bias correction
Falls back to hardcoded defaults if files are missing or empty.
