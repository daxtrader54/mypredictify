...
```
Step 1:  npm run sync-results                              (fetch match scores from SportMonks)
Step 2:  npx tsx scripts/evaluate-gameweek.ts --gw GW{n}   (produce evaluation.json)
Step 3:  npx tsx scripts/update-elo-batch.ts --gw GW{n}    (update Elo ratings)
Step 4:  npx tsx scripts/adjust-weights.ts                  (tune model weights — needs 2+ GWs)
Step 5:  npx tsx scripts/update-poisson-calibration.ts      (track goal prediction bias)
Step 6:  npx tsx scripts/update-performance-log.ts --gw GW{n}  (append to cumulative log)
Step 7:  npx tsx scripts/track-skill-performance.ts --gw GW{n} (link versions to accuracy)
Step 8:  [Claude] Generate/prune heuristics from patterns
Step 9:  [Claude] Propose skill/fragment improvements if warranted (versioned)
Step 10: /generate-report (re-run with results data)
  ↓ (commit all: results, evaluation, updated memory/, report)
Step 11: Git push
```
